USE [DADOSPRO]
GO
/****** Object:  StoredProcedure [dbo].[SP_REL_MOVIMENTO_DIARIO]    Script Date: 10/08/2021 17:32:43 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
/******************************************************************************
* PROCEDURE : SP_REL_MOVIMENTO_DIARIO  		  								  *
...
EXEC SP_REL_MOVIMENTO_DIARIO '20150501','20150531','','ZZZZZZZZZZZZZZZ','  ', '  ', '02', '3'
*/

ALTER PROCEDURE [dbo].[SP_REL_MOVIMENTO_DIARIO]
	@DINICIO VARCHAR(10),
	@DFINAL VARCHAR(10),
...
BEGIN
	...
	END
	SET @CNOMEEMPRES = 'DAIHATSU - MATRIZ'
	IF @CFILIAL='02'
	BEGIN
		SET @CNOMEEMPRES = 'DAIHATSU - NACOES UNIDAS'
	END
	...
	INSERT INTO #TEMP_SALDO_INICIAL_DE_SB6
      (
		...
		FROM SB6010 SB6 WITH(NOLOCK)
			INNER JOIN SB1010 SB1 WITH(NOLOCK) ON SB1.B1_COD=SB6.B6_PRODUTO AND SB1.D_E_L_E_T_ 	<> '*' 
		WHERE   
			SB6.D_E_L_E_T_ 	 	<> '*' 
	...
	INSERT INTO #TEMP_MOVIMENTOS_DE_SB6
      (
		...
		FROM SB6010 SB6 WITH(NOLOCK)
			INNER JOIN SB1010 SB1 WITH(NOLOCK) ON SB1.B1_COD=SB6.B6_PRODUTO AND SB1.D_E_L_E_T_ 	<> '*' 
			INNER JOIN SF4010 SF4 WITH(NOLOCK) ON F4_CODIGO = B6_TES AND SF4.D_E_L_E_T_	= ' ' 
	...
	INSERT INTO #TEMP_MOVIMENTOS_DE_SB6
      (
		...
		WHERE   
			...
			AND SB6.B6_PODER3	=  'D'
			AND EXISTS (
						SELECT 'X' FROM SB6010 AUX WITH(NOLOCK)
						WHERE  
	...
	UPDATE #TEMP_MOVIMENTOS_DE_SB6 SET VL_CUSTO = CAST((SELECT dbo.RET_CUSTO_ATUAL_CT('01', @CFILIAL, '21', CONVERT(VARCHAR(8), GETDATE(), 112), B6_PRODUTO)) AS DECIMAL(12,4))
	...
	BEGIN
		INSERT INTO #TEMP_SALDO_INICIAL
		  (
			...
			VL_CUSTO	,
			B1_ITEMCC
		  ) EXEC [SP_REL_MOVIMENTO_DIARIO_SALDO_SB9] @DINICIO, @DFINAL, @CPRODUTOINI, @CPRODUTOFIN,	@CLOCALINI, @CTIPOINI, @CFILIAL
	END
	...
	BEGIN
		INSERT INTO #TEMP_SALDO_INICIAL
		  (
			...
			VL_CUSTO	,
			B1_ITEMCC
		  ) EXEC [SP_REL_MOVIMENTO_DIARIO_SALDO_SB2] @DINICIO, @DFINAL, @CPRODUTOINI, @CPRODUTOFIN,	@CLOCALINI, @CTIPOINI, @CFILIAL
	END
	...
	INSERT INTO #TEMP_MOVIMENTOS_DE_SD1
      (
		...
		FROM SD1010 SD1 WITH(NOLOCK)
			INNER JOIN SB1010 SB1 WITH(NOLOCK) ON SB1.B1_COD=SD1.D1_COD AND SB1.D_E_L_E_T_ 	<> '*' 
			INNER JOIN SF4010 SF4 WITH(NOLOCK) ON F4_CODIGO = D1_TES AND SF4.D_E_L_E_T_	= ' ' AND SF4.F4_ESTOQUE='S' 
			LEFT OUTER JOIN ZAJ010 ZAJ WITH(NOLOCK) ON ZAJ_COD = F4_META AND ZAJ.D_E_L_E_T_	= ' '
		WHERE   
			SD1.D_E_L_E_T_ 	 	<> '*' 
	...
	INSERT INTO #TEMP_MOVIMENTOS_DE_SD2
      (
		...
		FROM SD2010 SD2 WITH(NOLOCK)
			INNER JOIN SB1010 SB1 WITH(NOLOCK) ON SB1.B1_COD=SD2.D2_COD AND SB1.D_E_L_E_T_ 	<> '*'
			INNER JOIN SF4010 SF4 WITH(NOLOCK) ON F4_CODIGO = D2_TES AND SF4.D_E_L_E_T_	= ' ' AND SF4.F4_ESTOQUE='S'
			LEFT OUTER JOIN ZAJ010 ZAJ WITH(NOLOCK) ON ZAJ_COD = F4_META AND ZAJ.D_E_L_E_T_	= ' '
		WHERE   
			SD2.D_E_L_E_T_ 	 	<> '*' 
	...
	INSERT INTO #TEMP_MOVIMENTOS_DE_SD3
      (
		...
		FROM SD3010 SD3 WITH(NOLOCK)
			INNER JOIN SB1010 SB1 WITH(NOLOCK) ON SB1.B1_COD=SD3.D3_COD AND SB1.D_E_L_E_T_=''
		WHERE   
			SD3.D_E_L_E_T_ 	 	<> '*' 
	...
	-- COMENTADO POR GILBERTO E CRIADA OUTRA LINHA PARA OBTER O CUSTO DE ACORDO COM O FECHAMENTO DO PRÓPRIO PROTHEUS, ATRAVÉS DA FUNÇÃO (RET_CUSTO_ATUAL) - 29/10/2014
	...
	UPDATE #TEMP_MOVIMENTOS_DE_SD1 SET VL_CUSTO = CAST((SELECT dbo.RET_CUSTO_ATUAL_CT('01', @CFILIAL, '21', CONVERT(VARCHAR(8), GETDATE(), 112), D1_COD)) AS DECIMAL(12,4))
	...
	UPDATE #TEMP_MOVIMENTOS_DE_SD2 SET VL_CUSTO = CAST((SELECT dbo.RET_CUSTO_ATUAL_CT('01', @CFILIAL, '21', CONVERT(VARCHAR(8), GETDATE(), 112), D2_COD)) AS DECIMAL(12,4))
	...
	UPDATE #TEMP_MOVIMENTOS_DE_SD3 SET VL_CUSTO = CAST((SELECT dbo.RET_CUSTO_ATUAL_CT('01', @CFILIAL, '21', CONVERT(VARCHAR(8), GETDATE(), 112), D3_COD)) AS DECIMAL(12,4))
	...
	UPDATE #TEMP_SALDO_INICIAL SET VL_CUSTO = CAST((SELECT dbo.RET_CUSTO_ATUAL_CT('01', @CFILIAL, '21', CONVERT(VARCHAR(8), GETDATE(), 112), B9_COD)) AS DECIMAL(12,4))

	--
	-- CRIA TABELA QUE RECEBE MOVIMENTOS ENTRE ARMAZENS E PRODUÇÃO
	--
    CREATE TABLE TEMP_ANALITICO_BM_DAIHATSU
      (
	...
	INSERT INTO TEMP_ANALITICO_BM_DAIHATSU
      (
		ID_LOCAL	,
		NM_ARMAZ	,
		B1_TIPO		,
	...
	INSERT INTO TEMP_MOVIMENTOS_BM
      (
		ID_LOCAL	,
		NM_ARMAZ	,
		B1_TIPO		,
	...
	DELETE TEMP_MOVIMENTOS_BM WHERE TT_QUANT=0
