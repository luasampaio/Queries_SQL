USE [DADOSPRO]
GO
/****** Object:  StoredProcedure [dbo].[SP_REL_MOVIMENTO_DIARIO_PROART]    Script Date: 10/08/2021 17:40:30 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
/******************************************************************************
* PROCEDURE : SP_REL_MOVIMENTO_DIARIO_PROART  		  						  *
...
EXEC SP_REL_MOVIMENTO_DIARIO_PROART '20150501','20150531','','ZZZZZZZZZZZZZZZ','  ', '  ', '01', '3'
*/
ALTER PROCEDURE [dbo].[SP_REL_MOVIMENTO_DIARIO_PROART]
	@DINICIO VARCHAR(10),
	@DFINAL VARCHAR(10),
...
BEGIN
	...
	--
    CREATE TABLE #TEMP_SALDO_INICIAL_DE_SB6_PROART
      (
	...
	INSERT INTO #TEMP_SALDO_INICIAL_DE_SB6_PROART
      (
		B6_PRODUTO	,
		B1_DESC		,
		B1_TIPO		,
		...
		INNER JOIN SB1030 SB1 WITH(NOLOCK)
			ON SB1.B1_COD = SB6.B6_PRODUTO 
			AND SB1.B1_FILIAL = SB6.B6_FILIAL
			AND SB1.D_E_L_E_T_ 	<> '*'
		WHERE   
			SB6.D_E_L_E_T_ 	 	<> '*' 
	...
	FROM #TEMP_SALDO_INICIAL_DE_SB6_PROART AUX
	...
	--
    CREATE TABLE #TEMP_MOVIMENTOS_DE_SB6_PROART
      (
		B6_PRODUTO	VARCHAR(15)		 NULL,
		B1_DESC		VARCHAR(50)		 NULL,
		B1_TIPO		VARCHAR(2)		 NULL,
	...
	INSERT INTO #TEMP_MOVIMENTOS_DE_SB6_PROART
      (
		B6_PRODUTO	,
		B1_DESC		,
		B1_TIPO		,
		...
		INNER JOIN SB1030 SB1 WITH(NOLOCK) 
		   ...
		   AND SB1.B1_FILIAL = SB6.B6_FILIAL
		   AND SB1.D_E_L_E_T_ 	<> '*'
		INNER JOIN SF4030 SF4 WITH(NOLOCK)
			ON F4_CODIGO = B6_TES 
		   AND SF4.D_E_L_E_T_ = ' ' 
		   AND SF4.F4_FILIAL  = SB6.B6_FILIAL
		LEFT OUTER JOIN ZAJ010 ZAJ WITH(NOLOCK)
		    ON ZAJ_COD = F4_META 
		   AND ZAJ.D_E_L_E_T_	= ' '
		WHERE   
			SB6.D_E_L_E_T_ 	 	<> '*' 
	...
	FROM #TEMP_MOVIMENTOS_DE_SB6_PROART AUX
	WHERE AUX.B6_LOCAL=' '
	...
	UPDATE AUX
		SET SLD_INICIAL = ISNULL( 
			(
				SELECT SUM(B6_QUANT) FROM #TEMP_SALDO_INICIAL_DE_SB6_PROART AUX_II 
				WHERE 
	...
	INSERT INTO #TEMP_MOVIMENTOS_DE_SB6_PROART
      (
		...
		B1_ITEMCC
			...
			B1_ITEMCC
		FROM #TEMP_SALDO_INICIAL_DE_SB6_PROART TMP
		WHERE NOT EXISTS (
						SELECT 'X' FROM #TEMP_MOVIMENTOS_DE_SB6_PROART AUX 
						WHERE
	...
	-- TRATAR AQUI PARA OBTER DA SB9, E QUANDO O PRODUTO NÃO EXISTIR NA SB9, OBTER O CUSTO DA SB2
	--UPDATE #TEMP_MOVIMENTOS_DE_SB6_PROART SET 
	--	VL_CUSTO = ISNULL(( SELECT DISTINCT CONVERT( DECIMAL(12,4) , B2_CM1 ) 
	--						FROM SB2030 SB2 WITH(NOLOCK)
	--						WHERE B6_PRODUTO = B2_COD  
	--						  AND B6_LOCAL   = B2_LOCAL  
	--						  AND B2_FILIAL  = @CFILIAL
	--						  AND SB2.D_E_L_E_T_=' '),0)
	-- BUSCA DO CUSTO PELA FUNAO RET_CUSTO_ATUAL_CT -- EM 12/06/2015 POR CT
	UPDATE #TEMP_MOVIMENTOS_DE_SB6_PROART SET VL_CUSTO = CAST((SELECT dbo.RET_CUSTO_ATUAL_CT('03', @CFILIAL, '21', CONVERT(VARCHAR(8), GETDATE(), 112), B6_PRODUTO)) AS DECIMAL(12,4))
	...
	--
    CREATE TABLE #TEMP_SALDO_INICIAL_PROART
      (
	...
	IF @NTIPOREL = '1'
	BEGIN
		INSERT INTO #TEMP_SALDO_INICIAL_PROART
		  (
			B9_COD		,
			B1_DESC		,
			B1_TIPO		,
			...
			VL_CUSTO	,
			B1_ITEMCC
		  ) EXEC [SP_REL_MOVIMENTO_DIARIO_SALDO_SB9_PROART] @DINICIO, @DFINAL, @CPRODUTOINI, @CPRODUTOFIN,	@CLOCALINI, @CTIPOINI, @CFILIAL
	END
	...
	BEGIN
		INSERT INTO #TEMP_SALDO_INICIAL_PROART
		  (
			...
			VL_CUSTO	,
			B1_ITEMCC	
		  ) EXEC [SP_REL_MOVIMENTO_DIARIO_SALDO_SB2_PROART] @DINICIO, @DFINAL, @CPRODUTOINI, @CPRODUTOFIN,	@CLOCALINI, @CTIPOINI, @CFILIAL
	END

	--
	-- Cria tabela que recebe movimentos de Entrada de NF
	--
    CREATE TABLE #TEMP_MOVIMENTOS_DE_SD1_PROART
      (
	...
	INSERT INTO #TEMP_MOVIMENTOS_DE_SD1_PROART
      (
		D1_COD		,
		B1_DESC		,
		B1_TIPO		,
		...
		LEFT OUTER JOIN ZAJ010 ZAJ WITH(NOLOCK)
			ON ZAJ_COD = F4_META AND ZAJ.D_E_L_E_T_	= ' '
	...
	--
    CREATE TABLE #TEMP_MOVIMENTOS_DE_SD2_PROART
      (
	...
	INSERT INTO #TEMP_MOVIMENTOS_DE_SD2_PROART
      (
		D2_COD		,
		B1_DESC		,
		B1_TIPO		,
		...
		INNER JOIN SB1030 SB1 
		   ...
		   AND SB1.B1_FILIAL = SD2.D2_FILIAL
		   AND SB1.D_E_L_E_T_ 	<> '*'
		INNER JOIN SF4030 SF4 WITH(NOLOCK)
	...
	--
    CREATE TABLE #TEMP_MOVIMENTOS_DE_SD3_PROART
      (
	...
	INSERT INTO #TEMP_MOVIMENTOS_DE_SD3_PROART
      (
		D3_COD		,
		B1_DESC		,
		B1_TIPO		,
		...
		INNER JOIN SB1030 SB1 WITH(NOLOCK)
		   ...
		   AND SB1.B1_FILIAL = SD3.D3_FILIAL
		   AND SB1.D_E_L_E_T_ 	<> '*' 
	...
	-- TRATAR AQUI PARA OBTER DA SB9, E QUANDO O PRODUTO NÃO EXISTIR NA SB9, OBTER O CUSTO DA SB2
	--UPDATE #TEMP_MOVIMENTOS_DE_SD1_PROART SET 
	--	VL_CUSTO = ISNULL(( SELECT DISTINCT CONVERT( DECIMAL(12,4) , B2_CM1 ) 
	--						FROM SB2030 SB2 WITH(NOLOCK)
	--						WHERE D1_COD = B2_COD  
	...
	--						  AND B2_FILIAL  = @CFILIAL
	--						  AND SB2.D_E_L_E_T_=' '),0)
	UPDATE #TEMP_MOVIMENTOS_DE_SD1_PROART SET VL_CUSTO = CAST((SELECT dbo.RET_CUSTO_ATUAL_CT('03', @CFILIAL, '21', CONVERT(VARCHAR(8), GETDATE(), 112), D1_COD)) AS DECIMAL(12,4))

	-- TRATAR AQUI PARA OBTER DA SB9, E QUANDO O PRODUTO NÃO EXISTIR NA SB9, OBTER O CUSTO DA SB2
	--UPDATE #TEMP_MOVIMENTOS_DE_SD2_PROART SET 
	--	VL_CUSTO = ISNULL(( SELECT DISTINCT CONVERT( DECIMAL(12,4) , B2_CM1 ) 
	--						FROM SB2030 SB2 WITH(NOLOCK) 
	--						WHERE D2_COD = B2_COD  
	...
	--						  AND B2_FILIAL  = @CFILIAL
	--						  AND SB2.D_E_L_E_T_=' '),0)
	UPDATE #TEMP_MOVIMENTOS_DE_SD2_PROART SET VL_CUSTO = CAST((SELECT dbo.RET_CUSTO_ATUAL_CT('03', @CFILIAL, '21', CONVERT(VARCHAR(8), GETDATE(), 112), D2_COD)) AS DECIMAL(12,4))

	-- TRATAR AQUI PARA OBTER DA SB9, E QUANDO O PRODUTO NÃO EXISTIR NA SB9, OBTER O CUSTO DA SB2
	--UPDATE #TEMP_MOVIMENTOS_DE_SD3_PROART SET 
	--	VL_CUSTO = ISNULL(( SELECT DISTINCT CONVERT( DECIMAL(12,4) , B2_CM1 ) 
	--						FROM SB2030 SB2 WITH(NOLOCK)
	--						WHERE D3_COD = B2_COD  
	...
	--						  AND B2_FILIAL  = @CFILIAL
	--						  AND SB2.D_E_L_E_T_ = ' '),0)
	UPDATE #TEMP_MOVIMENTOS_DE_SD3_PROART SET VL_CUSTO = CAST((SELECT dbo.RET_CUSTO_ATUAL_CT('03', @CFILIAL, '21', CONVERT(VARCHAR(8), GETDATE(), 112), D3_COD)) AS DECIMAL(12,4))
	

	DELETE #TEMP_SALDO_INICIAL_PROART	   
	WHERE ISNULL(B9_QINI,0) + ISNULL(D1_QUANT,0) + ISNULL(D2_QUANT,0) + ISNULL(D3_QUANT,0) = 0
	  AND NOT EXISTS (SELECT 'X' FROM #TEMP_MOVIMENTOS_DE_SD1_PROART XSD1 WHERE B9_COD=D1_COD AND B9_LOCAL=D1_LOCAL)
	  AND NOT EXISTS (SELECT 'X' FROM #TEMP_MOVIMENTOS_DE_SD2_PROART XSD2 WHERE B9_COD=D2_COD AND B9_LOCAL=D2_LOCAL)
	  AND NOT EXISTS (SELECT 'X' FROM #TEMP_MOVIMENTOS_DE_SD3_PROART XSD3 WHERE B9_COD=D3_COD AND B9_LOCAL=D3_LOCAL)

	-- TRATAR AQUI PARA OBTER DA SB9, E QUANDO O PRODUTO NÃO EXISTIR NA SB9, OBTER O CUSTO DA SB2
	--UPDATE #TEMP_SALDO_INICIAL_PROART SET 
	--	VL_CUSTO = ISNULL(( SELECT DISTINCT CONVERT( DECIMAL(12,4) , B2_CM1 ) 
	--						FROM SB2030
	--						WHERE B9_COD=B2_COD  
	...
	--						  AND B2_FILIAL  = @CFILIAL
	--						  AND D_E_L_E_T_=' '),0)
	UPDATE #TEMP_SALDO_INICIAL_PROART SET VL_CUSTO = CAST((SELECT dbo.RET_CUSTO_ATUAL_CT('03', @CFILIAL, '21', CONVERT(VARCHAR(8), GETDATE(), 112), B9_COD)) AS DECIMAL(12,4))

	--
	-- Cria tabela que recebe movimentos entre armazens e produção
	--
    CREATE TABLE TEMP_ANALITICO_BM_PROART
      (
	...
	INSERT INTO TEMP_ANALITICO_BM_PROART
      (
		ID_LOCAL	,
		NM_ARMAZ	,
		B1_TIPO		,
		...
		FROM (
				SELECT
					...
					END AS VL_CUSTO,
					B1_ITEMCC
				FROM #TEMP_MOVIMENTOS_DE_SD1_PROART
			UNION ALL
				SELECT
					...
					END AS VL_CUSTO,
					B1_ITEMCC
				FROM #TEMP_MOVIMENTOS_DE_SD2_PROART
			UNION ALL
				SELECT
					...
					B1_ITEMCC
				FROM #TEMP_MOVIMENTOS_DE_SD3_PROART
			UNION ALL
				SELECT
					...
					B1_ITEMCC
				FROM #TEMP_SALDO_INICIAL_PROART	   
			UNION ALL
				SELECT 
					...
					B1_ITEMCC
				FROM #TEMP_MOVIMENTOS_DE_SB6_PROART 
				WHERE B6_TIPO='E'
			UNION ALL
				SELECT DISTINCT
					B6_LOCAL	AS ID_LOCAL		,
			...
			-- INCLUÍDO POR GILBERTO (PARA CONTEMPLAR O ESTOQUE DO ARMAZÉM 25, QUE PERTENCE A PROART)
			UNION ALL
				SELECT   A.ID_LOCAL
						,A.B1_TIPO
						,A.L_T
						...
						,A.TT_CUSTO
						,A.UN_NEGOCIO
						--, (A.TT_QUANT * ZAI.ZAI_CUSTO) 'TT_CUSTO'
				FROM TEMP_ANALITICO_BM_DAIHATSU A -- TABELA CRIADA NO DTS (ANALITICO_MOVTO_ARMAZENS_PARA_BM)
				--	,ZAI030 ZAI-- TABELA DE CUSTO FIXO PARA ITENS QUE ESTÃO NO 
				WHERE A.ID_LOCAL = '25'
				  --AND A.ID_PRODUTO = ZAI.B2_COD  
				  --AND A.ID_LOCAL   = ZAI.B2_LOCAL  
				  --AND SB2.D_E_L_E_T_=' '
			-- FIM INCLUSÃO GILBERTO
	...
	DELETE TEMP_ANALITICO_BM_PROART WHERE TT_QUANT=0

	--
	-- Cria tabela que recebe movimentos entre armazens e produção para Balanço de Massas - DTS
	--
    CREATE TABLE TEMP_MOVIMENTOS_BM_PROART
      (
		ID_LOCAL	VARCHAR(2)		 NULL,
		NM_ARMAZ	VARCHAR(50)		 NULL,
		B1_TIPO		VARCHAR(2)		 NULL,
	...
	INSERT INTO TEMP_MOVIMENTOS_BM_PROART
      (
		ID_LOCAL	,
		NM_ARMAZ	,
		B1_TIPO		,
	...
	FROM
		...
		UNION ALL
			SELECT   A.B1_TIPO		AS B1_TIPO
					...
					,0				AS DE_QUANT	
					,0				AS DE_CUSTO	
					,A.TT_QUANT		AS EM_QUANT
					,A.TT_CUSTO		AS EM_CUSTO
					,A.UN_NEGOCIO
					--, (A.TT_QUANT * ZAI.ZAI_CUSTO) AS EM_CUSTO
			FROM TEMP_ANALITICO_BM_DAIHATSU A -- TABELA CRIADA NO DTS (ANALITICO_MOVTO_ARMAZENS_PARA_BM)
			--	,ZAI030 ZAI-- TABELA DE CUSTO FIXO PARA ITENS QUE ESTÃO NO 
			WHERE A.ID_LOCAL = '25'
			  --AND A.ID_PRODUTO = ZAI.B2_COD  
	...
	DELETE TEMP_MOVIMENTOS_BM_PROART WHERE TT_QUANT=0
