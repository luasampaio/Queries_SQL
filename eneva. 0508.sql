...
SELECT  T1.[Customer]
	  ...
	  ,T2.AREA AS AREATI
	  ,T2.DiasViolados 
  FROM [dbo].[FactTickets] AS T1 LEFT JOIN 
  [dbo].[TicketsViolated] AS T2 ON T1.TicketID=T2.ticket_id and T1.TicketNumber=T2.TicketNumber
  WHERE MONTH(T1.StartTicket)=6 AND YEAR(T1.StartTicket)=2021
  AND T1.Customer = 'ENEVA'
  and T1.TicketNumber = '2201975'